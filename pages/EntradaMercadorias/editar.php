...
// Manipulação de atualização de dados
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        foreach ($_POST['produtos'] as $produto) {
            ...
            if (!empty($produto['id']) && !empty($produto['nome']) && !empty($produto['quantidade'])) {
                $dados['produtos'][] = [
                    ...
                    'estoque' => (int) $produto['estoque'] // Estoque atual do produto
                ];
            }
        }
    }
  ...
  exit;
    ...
    if ($returnUpdate) {
        echo notify('success', "Entrada de mercadoria atualizada com sucesso!");
        echo '<meta http-equiv="refresh" content="2; url=' . $url . '!/' . $link[1] . '/listar">';
    } else {
        echo notify('danger', "Erro ao atualizar a entrada de mercadoria.");
    }
}
...
<div class="card">
    <div class="card-header bg-primary text-white d-flex justify-content-between align-items-center">
        <h3 class="card-title">Editar Entrada de Mercadoria</h3>
        ...
        <form method="POST" action="<?php echo "{$url}!/{$link[1]}/{$link[2]}/{$id}" ?>" class="needs-validation" novalidate>
            <div class="row g-3">
                ...
                <div class="col-lg-4">
                    ...
                    <input type="text" class="form-control" id="nf_fiscal" name="nf_fiscal" value="<?= $return['nf_fiscal'] ?>" required>
                </div>
                <div class="col-lg-4">
                    <label for="data_pedido" class="form-label">Data do Pedido</label>
                    <input type="date" class="form-control" id="data_pedido" name="data_pedido" value="<?= $return['data_pedido'] ?>" required>
                </div>
                <div class="col-lg-4">
                    <label for="fornecedor_id" class="form-label">Fornecedor</label>
                    <select class="form-select" id="fornecedor_id" name="fornecedor_id" required>
                        <option value="" disabled>Selecione um fornecedor</option>
                        <?php foreach ($controller->listarFornecedores() as $fornecedor): ?>
                            <option value="<?php echo $fornecedor['id']; ?>" <?php echo $fornecedor['id'] == $return['fornecedor_id'] ? 'selected' : ''; ?>>
                ...
                <div class="col-lg-4">
                    ...
                    <input type="date" class="form-control" id="data_prevista_entrega" name="data_prevista_entrega" value="<?= $return['data_prevista_entrega'] ?>" required>
                </div>
                <div class="col-lg-4">
                    <label for="valor" class="form-label">Valor</label>
                    <input type="number" step="0.01" class="form-control" id="valor" name="valor" value="<?= $return['valor'] ?>" required>
                </div>
                <div class="col-lg-4">
                    <label for="transportadora" class="form-label">Transportadora</label>
                    <input type="text" class="form-control" id="transportadora" name="transportadora" value="<?= $return['transportadora'] ?>" required>
                </div>
                <div class="col-lg-12">
                    <label for="observacoes" class="form-label">Observações</label>
                    <textarea class="form-control" id="observacoes" name="observacoes" rows="3"><?= $return['observacoes'] ?></textarea>
                </div>
                ...
                <div class="col-lg-12">
                    ...
                    <div id="product-list">
                        <?php foreach ($return['produtos'] as $index => $produto): ?>
                            <div class="row g-3 align-items-end product-item mb-2">
                                <div class="col-lg-6">
                                    <input type="text" class="form-control product-input" readonly
                                        value="<?= htmlspecialchars($produto['nome_produto']) ?>"
                                        data-index="<?= $index ?>">
                                    <input type="hidden" name="produtos[<?= $index ?>][id]" class="product-id" value="<?= $produto['produto_id'] ?>">
                                    <input type="hidden" name="produtos[<?= $index ?>][estoque]" class="product-stock" value="<?= $produto['estoque'] ?>">
                                </div>
                                <div class="col-lg-4">
                                    <input type="number" class="form-control" name="produtos[<?= $index ?>][quantidade]" value="<?= $produto['quantidade'] ?>" required>
                                </div>
                                <div class="col-lg-2">
                    ...
                    <div class="col-lg-12 mt-3">
                        <button type="button" class="btn btn-success btn-add">Adicionar Produto</button>
                    </div>
                </div>
            </div>
