...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($return) {
        echo notify('success', "Cheque atualizado com sucesso!");
        echo '<meta http-equiv="refresh" content="2; url=' . $url . '!/' . $link[1] . '/listar">';
    } else {
        echo notify('danger', "Erro ao atualizar o cheque.");
    }
}
...
<div class="card">
  ...
  <div class="card-body">
    <form method="POST" action="<?php echo "{$url}!/{$link[1]}/{$link[2]}/{$id}" ?>" class="needs-validation" novalidate>
      <div class="row g-3">
        <div class="col-lg-6">
          <label class="form-label">Nome do Cheque</label>
          <input type="text" class="form-control" name="nome_cheque" value="<?= htmlspecialchars($cheque['nome_cheque']) ?>" required>
        </div>
        <div class="col-lg-6">
          ...
          <select class="form-select" name="max_parcelas" required>
            ...
            <?php for ($i = 1; $i <= 12; $i++): ?>
              <option value="<?= $i ?>" <?= ($cheque['max_parcelas'] ?? 0) == $i ? 'selected' : '' ?>><?= $i ?>x</option>
            <?php endfor; ?>
          </select>
        </div>
        ...
        <?php for ($i = 1; $i <= 12; $i++): ?>
          <div class="col-lg-3">
            ...
            <input type="number" step="0.01" class="form-control" name="juros_parcela_<?= $i ?>" value="<?= htmlspecialchars($cheque["juros_parcela_$i"] ?? '') ?>">
          </div>
        <?php endfor; ?>
        <div class="col-lg-12">
